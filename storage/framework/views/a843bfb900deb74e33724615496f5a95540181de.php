...
<nav class="nav-second">
    <ul>
      ...
      <li>
          <a class="" href="/client-faq"><i class="fa fa-question-circle" aria-hidden="true"></i>よくある質問</a>
      </li>
      <li>
          <a class="" href="/"><i class="fa fa-info-circle" aria-hidden="true"></i>サービスの流れ</a>
      </li>
    　<li class="">
          <a class="" href="/chat"><i class="fa fa-comments" aria-hidden="true"></i>メッセージ<span id="notification"></span></a>
      </li>
    　<li>
...
<!-- chat starts here -->
  ...
  <!--<div class="chat">
      ...
      <div class="chat-open">
          ...
          <div class="input">
              <textarea id="typedMessage"></textarea><a class="send" href="#" onclick="send_typed_message(event)">送信</a>
          </div>
          </div>
          </div>
      </div>
...
function back_company_list(){
    ...
    $.get("/check-messages",
          ...
          function(data,status){
            ...
            for(count=0;count<data.length;count++)
            {
              ...
              var list_item='<li id="item-'+data[count][1]+'"><a '+online_class+' href="#" onclick="open_company_chats('+data[count][1]+',\''+data[count][0]+'\','+online_status+',event)">'+data[count][0]+'<span class="unread">'+unread+'</span><i class="fa fa-chevron-right" aria-hidden="true"></i></a></li>';
              var current_unread=$('#unread'+data[count][1]).text();
              $('#company-list').append(list_item);
            }
    });
    }
  function open_company_chats(id,name,online_status,event)
    {
		event.preventDefault();
      chatting_company=id;
  ...
  function send_typed_message(event) {
	  event.preventDefault();
