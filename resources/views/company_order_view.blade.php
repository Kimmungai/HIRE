...
<div class="place-bid">
    ...
    <form action="/bid_with_message" method="POST">
        ...
        <label>金額:</label>
        <input name="bid-price" type="number" onkeyup="calculate_charges(this.value,{{$consumption_tax_rate}},{{$excia_commission}})" value="<?php if(isset($price)){echo $price;} ?>" required <?php if(Auth::user()->admin_approved==0 || $orders[0]['bid_status']==1){echo '無効';} ?>>
        <p class="youget"></p>
        <p class="handling"></p>
        <p class="tax"></p>
        <label>メッセージ:</label>
